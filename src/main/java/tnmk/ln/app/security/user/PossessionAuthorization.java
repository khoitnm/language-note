...
public class PossessionAuthorization {
    public void validateCanRemovePossession(User user, Possession possession) {
        if (!canRemovePossession(user, possession)) {
            throw new AuthorizationServiceException(String.format("No permission. %s belongs to another one, you cannot delete it. TopicId: %s, OwnerId: %s, UserId: %s", possession.getClass().getSimpleName(), possession.getId(), possession.getOwner().getId(), user.getId()));
        }
    }
